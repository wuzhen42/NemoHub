...
from PySide6.QtWidgets import QApplication, QFrame, QVBoxLayout, QHBoxLayout, QTableWidgetItem
from qfluentwidgets import SettingCard, InfoBar, InfoBarPosition, TableWidget, PrimaryPushButton, PushButton, MessageBox, PrimaryPushSettingCard
from qfluentwidgets import FluentIcon as FIF

from app import utils
from app.api import ApiError
from app.license_store import AssignmentChanged, LicenseStore, PendingIssuance, as_datetime, next_month, read_license
...
class LicenseWidget(QFrame):
    ...
    def _refresh_if_visible(self):
        if self.isVisible() and self.worker is None and QApplication.activeModalWidget() is None:
            self.fetchSeatLicense()
    ...
    def _error(self, error):
        ...
        elif isinstance(error, ApiError) and error.status == 404 and str(error) == 'Not Found':
            text = self.tr('This server is missing this license feature. Please ask your administrator to update the server.')
        elif isinstance(error, AssignmentChanged):
            text = self.tr('The machine assignment changed. Please try again. A new activation costs one month; your offline file is unchanged.')
    ...
    def _confirm(self, title, text):
        return MessageBox(title, text, self.window()).exec()

    def _confirm_overwrite(self, seat):
        if not self.seatData or self.seatData['seat_id'] == seat['id']:
            return True
        return self._confirm(self.tr('Replace local license'), self.tr(
            'Replace the local license from another seat?'
        ))
    ...
    def activateSeatLicense(self):
        ...
        if not self.api.is_subaccount and replacement:
            self._error(ApiError(403, self.tr('This seat has been activated on another machine.')))
            return
        text = self.tr(
            'Activate on this machine for one month?\n'
            'Available balance: {months} month(s).'
        ).format(months=seat['months'])
        if replacement and self.api.is_subaccount:
            text += '\n\n' + self.tr(
                'The old offline license stays valid until renewal.\n'
                'Issued time is not refunded.'
            )
    ...
    def refreshSeatLicense(self):
        ...
        text = self.tr(
            'Use one month to renew until {date}?\n'
            'Early renewal also costs one month. No refunds.'
        ).format(date=end.astimezone().strftime('%Y-%m-%d'))
    ...
    def retryPendingLicense(self):
        try:
            ...
            if not seat:
                raise PendingIssuance(self.tr('Sign in with the original subaccount and reopen the License tab to recover this request.'))
            text = self.tr(
                'Retry this request?\n'
                'An already-issued license will be downloaded without another charge.'
            )
            if self._confirm_overwrite(seat) and self._confirm(self.tr('Retry pending request'), text):
    ...
    def deactivateSeatLicense(self):
        seat = self.selectedSeat()
        if (not seat or self.api.is_studio_owner or self.worker
                or not (seat.get('fingerprint') or seat.get('hostname'))):
            return
        text = self.tr(
            'Clear the assignment on {machine}?\n'
            'Unused months stay. Reactivation costs one month.\n'
            'A matching local file is removed. Other offline copies stay valid until renewal.'
        ).format(machine=seat.get('hostname') or self.tr('Unknown'))
        if self._confirm(self.tr('Clear license assignment'), text):
    ...
    def updateLicenseCard(self):
        ...
        # Clearing an owned assignment requires neither the old machine nor a
        # local file (or even a fingerprint for this computer).
        assigned = bool(seat and (seat.get('fingerprint') or seat.get('hostname')))
        self.buttonDeactivate.setEnabled(assigned and self.worker is None
                                         and not self.api.is_studio_owner)
    ...
    def setup(self):
        ...
        self.buttonDeactivate = PrimaryPushButton(self.tr('Clear'))
        self.buttonActivate = PrimaryPushButton(self.tr('Activate'))
        for button in (self.buttonRefresh, self.buttonDownload, self.buttonDeactivate, self.buttonActivate):
            buttons.addWidget(button)
        self.buttonDownload.setVisible(self.api.is_subaccount)
        self.buttonDeactivate.setVisible(not self.api.is_studio_owner)
